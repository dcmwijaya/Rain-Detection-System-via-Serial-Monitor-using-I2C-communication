...
String sendData; // string variable used to send data to Arduino Mega
...
void sensor(){
  ...
  if(sensorState == LOW){ // sensor detected wet then
    sendData = "Rain Warning"; // print to serial monitor
  }
  if(sensorState == HIGH){ // sensor detected dry then
    sendData = "Not Raining"; // print to serial monitor
  }
}

// Method: dataRetrieval
void dataRetrieval(){
  Wire.requestFrom(slaveAddress,dataretrievalSize); // read data from Master
...
void dataTransmission(){
  ...
  Wire.write(data,sizeof(data)); // send data to Master
